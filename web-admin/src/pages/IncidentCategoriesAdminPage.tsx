...
export function IncidentCategoriesAdminPage(): ReactElement {
  ...
  const load = useCallback(async () => {
    try {
      const res = await apiFetch<{ items: Category[] }>('/incident-categories');
      setItems(res.items);
    } catch (e) {
  ...
  async function create(ev: FormEvent): Promise<void> {
    ...
    setMsg(null);
    try {
      await apiFetch('/admin/incident-categories', { method: 'POST', json: { label } });
      setLabel('');
  ...
  async function remove(id: string): Promise<void> {
    ...
    setMsg(null);
    try {
      await apiFetch(`/admin/incident-categories/${id}`, { method: 'DELETE' });
      await load();
    } catch (e) {
      setErr(e instanceof Error ? e.message : 'Erreur');
    }
  ...
  return (
    <Card className="m-6">
      <CardHeader>
        <CardTitle className="text-xl">Catégories d'incidents</CardTitle>
        <CardDescription>
          Référentiel des catégories proposées aux habitants pour signaler un incident.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        <form className="flex flex-wrap items-center gap-2" onSubmit={(e) => void create(e)}>
          <Input
            ...
            placeholder="Nom de la catégorie (ex. Sécurité)"
            required
          />
        ...
        {items.length === 0 ? (
          <p className="text-muted-foreground">
            Aucune catégorie. Les habitants ne peuvent pas signaler d'incident tant qu'il n'en existe pas.
          </p>
        ) : (
